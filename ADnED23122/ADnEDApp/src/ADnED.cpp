...
ADnED::ADnED(const char *portName, int maxBuffers, size_t maxMemory, int debug)
  : ADDriver(portName,
    ...
    m_debug(debug),group_num(0),tof_num(0),pixel_num(0)
{
  ...
  createParam(ADnEDHdfFileNumberParamString,    asynParamInt32,    &ADnEDHdfFileNumberParam);
  ...
  for (int det=0; det<=s_ADNED_MAX_DETS; det++) {
    ...
    paramStatus = ((setStringParam(det, ADnEDHdfFileTemplateParam, "_%5.5d.h5") == asynSuccess) && paramStatus);
    paramStatus = ((setIntegerParam(det, ADnEDHdfFileNumberParam, 0) == asynSuccess) && paramStatus);
...
void ADnED::eventHandler(shared_ptr<epics::pvData::PVStructure> const &pv_struct, epicsUInt32 channelID)
{
  ...
  if (pixelsPtr && tofPtr) {
    ...
    sprintf(group_name, "pulse_%5.5d", group_num);
    sprintf(pixel_name, "pixel_%5.5d", pixel_num);
    sprintf(tof_name, "tof_%5.5d", tof_num);
    ...
    getIntegerParam(ADnEDHdfWriteModeParam, &hdfFileWriteMode);

    //getIntegerParam(ADnEDHdfFileNumberParam, &file_num);

    switch(hdfFileWriteMode){
      case 0:
        cout << " the hdfFileNumber::: " << file_num <<endl;
        hdfFileNumber = static_cast<epicsUInt32>(file_num);
        status |= setIntegerParam(ADnEDHdfFileNumberParam, file_num);
        
        cout << " the status::: " << status <<endl;
        strcat(hdfFullFileName,hdfFileTemplate);
        sprintf(single_hdf_file_name, hdfFullFileName, file_num);
        nxstat = NXopen (single_hdf_file_name, NXACC_CREATE5, &file_id);
        NXmakegroup (file_id, "GPPD Monitor", "NXentry");
        NXopengroup (file_id, "GPPD Monitor", "NXentry");

         /* out the pulse id data         
          NXmakedata (file_id, "pulse_ID", NX_UINT64, 1 , &iter_pixel_num);
        ...
        break;
      case 1:
        cout<< "the capture modes" << endl; 
        break;
      case 2:
        cout << " the  pxiel_num::: " << pixel_num <<endl;
        if(pixel_num ==0)
        { 
          nxstat = NXopen (hdfFullFileName, NXACC_CREATE5, &file_id);
          if (nxstat == NX_ERROR) {
             asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
            "Error cannot open file NXfile.h5\n");
             }
         }
        nxstat= NXreopen (file_id, &clone_fileid);
        if (nxstat == NX_ERROR) {
            asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
            "Error cannot open file ****.h5\n");
            }
           
        NXmakegroup (clone_fileid, group_name, "NXentry");
        NXopengroup (clone_fileid, group_name, "NXentry");
            
             /* /* out the pulse id data           
              NXmakedata (clone_fileid, "pulse_ID", NX_UINT64, 1 , &iter_pixel_num);
                NXopendata (clone_fileid, "pulse_ID");
                  NXputdata (clone_fileid, pulseIDCopy);
                  //NXputattr (clone_fileid, "units", " ", 7, NX_CHAR);
                NXclosedata (clone_fileid); 
              */
                /* Output the pxiel ID */
          NXmakedata (clone_fileid, pixel_name, NX_UINT64, 1 , &dim);
          NXopendata (clone_fileid, pixel_name);
          NXputdata (clone_fileid, pixelsDataCopy);
          //NXputattr (clone_fileid, "units", " ", 7, NX_CHAR);
          NXclosedata (clone_fileid);
              
          NXmakedata (clone_fileid, tof_name, NX_UINT64, 1, &dim);
          NXopendata (clone_fileid, tof_name);
          NXputdata (clone_fileid, tofDataCopy);
          NXputattr (clone_fileid, "units", "microseconds", 12, NX_CHAR);
          NXclosedata (clone_fileid);
              
            // NXclosegroup (clone_fileid);
        NXclosegroup (clone_fileid);
        NXclose (&clone_fileid);     
