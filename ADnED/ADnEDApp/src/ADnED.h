...
class ADnED : public ADDriver {
 ...
 public:
  ...
  /* These arhreeAttributee the methods that we override from asynPortDriver */
  ...
  // new structure para
  int64_t start_every_slap;  
  int64_t slab_start[1], slab_size[1];
 ...
 //int unlimited_dims_three[2]={NX_UNLIMITED,3};
  ...
  uint64_t m_eventIndex[1]; 
  uint64_t  m_eventPulseTime[1]; 
  // storage flag
  int storageFlag;
